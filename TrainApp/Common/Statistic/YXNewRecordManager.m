...
@implementation YXNewRecordManager

+ (instancetype)sharedInstance{
    static YXNewRecordManager *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[YXNewRecordManager alloc] init];
...
//    }
    ...
    YXNewRecordManager *manager = [YXNewRecordManager sharedInstance];
...
- (void)checkInstantRecordAndStartUpload{
    if (!self.isInstantRecordProcessing && self.instantRecordArray.count>0) {
        ...
        __weak YXNewRecordManager *wself = self;
