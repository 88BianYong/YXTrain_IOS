    ...
    if ([UIImagePickerController isSourceTypeAvailable:sourceType]) {
        ...
        if (sourceType == UIImagePickerControllerSourceTypeCamera) {
            ...
            if (authStatus == AVAuthorizationStatusDenied
                ...
                [(YXBaseViewController *)viewController showToast:@"相机权限受限\n请在设置-隐私-相机中开启"];
                return;
            }
            [viewController presentViewController:self.imagePickerController animated:YES completion:nil];
        }else{
            ALAuthorizationStatus author = [ALAssetsLibrary authorizationStatus];
            if (author == kCLAuthorizationStatusRestricted || author ==kCLAuthorizationStatusDenied){
                [(YXBaseViewController *)viewController showToast:@"相册权限受限\n请在设置-隐私-相册中开启"];
                return;
            }
            self.completion(nil);
        }
    } else {
        [(YXBaseViewController *)viewController showToast:@"设备不支持拍照功能！"];
    }
}
