//
//  YXLearningStageCell_DeYang17.m
//  TrainApp
//
...
@interface YXLearningProcesView : UIView
@property (nonatomic, strong) UIImageView *imageView;
@property (nonatomic, strong) UILabel *nameLable;
@end
@implementation YXLearningProcesView
...
@interface YXLearningStageCell_DeYang17 ()
@property (nonatomic, strong) YXGradientView *gradientView;
@end

@implementation YXLearningStageCell_DeYang17
...
- (void)setupProcesButton:(NSArray<UIButton *> *)procesButtons {
    [procesButtons enumerateObjectsUsingBlock:^(UIButton * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ((idx / 4 % 2) == 0) {
...
#pragma mark - set
- (void)setToolExamineVoLists:(NSArray<__kindof ExamineDetailRequest_17Item_Examine_Process_ToolExamineVoList *> *)toolExamineVoLists {
    _toolExamineVoLists = toolExamineVoLists;
    ...
    [_toolExamineVoLists enumerateObjectsUsingBlock:^(__kindof ExamineDetailRequest_17Item_Examine_Process_ToolExamineVoList * _Nonnull tool, NSUInteger idx, BOOL * _Nonnull stop) {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.tag = idx + 1;
        WEAK_SELF
        [[button rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
            STRONG_SELF;
            BLOCK_EXEC(self.learningExamineProcesToolCompleteBlock,tool,idx);
        }];
        [self.contentView addSubview:button];
        YXLearningProcesView *procesView = [[YXLearningProcesView alloc] init];
        procesView.userInteractionEnabled = NO;
        procesView.nameLable.text = tool.name;
        procesView.imageView.image = [UIImage imageNamed:@"当前"];
        [button addSubview:procesView];
        [procesView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.center.equalTo(button);
            make.width.equalTo(button.mas_width);
        }];
        [buttonMutableArray addObject:button];
    }];
    for (int i = 1; i * 4 <= _toolExamineVoLists.count; i ++) {
        UIView *lineView = [[UIView alloc] init];
        lineView.backgroundColor = [UIColor colorWithHexString:@"eceef2"];
    ...
    [self setupProcesButton:buttonMutableArray];
}
