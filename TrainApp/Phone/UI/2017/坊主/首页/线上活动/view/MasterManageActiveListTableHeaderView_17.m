//
//  MasterManageActiveListTableHeaderView_17.m
//  TrainApp
//
...
@interface MasterManageActiveListTableHeaderView_17 ()
...
@end
@implementation MasterManageActiveListTableHeaderView_17
...
- (void)setScheme:(MasterManagerSchemeItem *)scheme {
    _scheme = scheme;
    [self.schemeView reloadMasterScheme:[NSString stringWithFormat:@"需要发布%@个活动(手机端暂不支持，请到PC端完成)",_scheme.amount] withFinishNum:_scheme.userfinishnum withAmount:_scheme.amount];
...
- (void)setupUI {
    ...
    [[self.explainButton rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        STRONG_SELF
        BLOCK_EXEC(self.masterActiveButtonBlock,self.explainButton);
    }];
